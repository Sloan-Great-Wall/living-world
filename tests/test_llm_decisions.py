...
def test_goal_tokens_drive_movement_weight():
    ...
    archive = Tile(
        tile_id="arch",
        display_name="Miskatonic Library",
        primary_pack="cthulhu",
        tile_type="restricted-archive",
    )
    study = Tile(
        tile_id="study",
        display_name="Armitage's Parlor",
        primary_pack="cthulhu",
...
def test_storyteller_alignment_bonus_applies_when_resident_has_goal():
    ...
    tpl = EventTemplate(
        event_kind="research-session", description="Agent conducts research", base_importance=0.2
    )
    tpl_other = EventTemplate(
        event_kind="coffee-break", description="A casual break", base_importance=0.2
    )
    st = TileStoryteller(
        tile, StorytellerConfig(), {"research-session": tpl, "coffee-break": tpl_other}
    )
    st.world = world
    st.goal_bonus = 2.0
...
def test_dialogue_reaction_parses_well_formed_json():
    ...
    assert result == {
        "reply": "I hear you.",
        "affinity_delta": 2,
        "belief_update": "listens carefully",
    }
...
def test_emergent_event_parse_basic():
    from living_world.agents.emergent import _clamp_proposal, _parse_proposal
...
def test_self_update_motivations_and_goal():
    ...
    a = Agent(
        agent_id="x", pack_id="scp", display_name="X", persona_card="...", current_goal="paperwork"
    )
    event = LegendEvent(
        event_id="e",
        tick=1,
