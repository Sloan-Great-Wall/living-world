...
TAG_TILE_AFFINITY: dict[str, dict[str, float]] = {
    ...
    "investigator": {
        "law-office": 2.5,
        "restricted-archive": 1.5,
        "scholar-study": 1.2,
        "decaying-port": 1.0,
        "wilderness-haunt": 0.8,
    },
    "reporter": {
        "law-office": 2.0,
        "decaying-port": 1.5,
        "wilderness-haunt": 1.2,
        "cult-hall": 0.4,
    },
...
class MovementPolicy:
    ...
    def _friend_tiles(self, agent) -> list[str]:
        ...
        friend_ids = [r.target_id for r in agent.relationships.values() if r.affinity >= 40]
        tiles: list[str] = []
    ...
    def tick(self) -> list[tuple[str, str, str, str]]:
        ...
        for agent in list(self.world.living_agents()):
            ...
            if use_llm and self.llm_advisor is not None:
                try:
