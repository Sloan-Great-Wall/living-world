...
class PostgresRepository:
    """psycopg3-backed repo. Requires `pip install -e '.[db]'`."""
                ...
                for rel in agent.relationships.values():
                    cur.execute(
                        """INSERT INTO relationships
                           (source_id, target_id, affinity, kind, last_interaction_tick)
                           VALUES (%s, %s, %s, %s, %s)
    ...
    def upsert_agent(self, agent: Agent) -> None:
        with self._conn.cursor() as cur:
            cur.execute(
                """INSERT INTO agents (agent_id, pack_id, display_name, persona_card,
                                       is_historical_figure, attributes, alignment, tags,
                                       life_stage, age, current_tile, current_goal,
                                       inventory, state_extra, last_tick, created_at_tick)
