"""Conscious-override layer — the "awake mind" on top of the subconscious rule machine.

Design (from user spec):
    Subconscious (rules) → always runs. Picks candidate events, rolls dice, applies stats.
    Conscious (LLM)      → activates probabilistically with event importance.
                           Given persona + goal + recent memory + the proposed event,
                           decides: APPROVE / ADJUST (new outcome) / VETO (doesn't happen).
